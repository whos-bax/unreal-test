...
def rename_and_move_files(folder_path, output_folder):
    ...
    for category in categories:
        category_folder = os.path.join(output_folder, f"scenario_1_{category}")
    ...
    for filename in files:
        ...
        if filename.lower().endswith((".mp4", ".avi", ".mov", ".mkv")) or "video" in filename.lower():
            ...
            if "day" in filename.lower():
                category = "day"
                new_filename = f"scenario_1_day_{counters[category]:010d}.png"
            elif "night" in filename.lower():
                category = "night"
                new_filename = f"scenario_1_night_{counters[category]:010d}.png"
            elif "hl" in filename.lower():  # hl 포함
                category = "hl"
                new_filename = f"scenario_1_hl_{counters[category]:010d}.png"
            else:
                # 카테고리에 속하지 않는 경우 스킵
            ...
            new_file_path = os.path.join(output_folder, f"scenario_1_{category}", new_filename)
...
# 파일이 저장된 폴더 경로
folder_path = "/Users/whosbax/Documents/GitHub/unreal-test/cesium1/Saved/MovieRenders/scenario_4"  # 이미지가 있는 폴더 경로를 입력하세요.
output_folder = "/Users/whosbax/Documents/GitHub/unreal-test/cesium1/Saved/MovieRenders/scenario_4"  # 결과 파일들이 저장될 상위 폴더 경로
